...
@export var npc_intro: Texture2D          # 1) introduzione
@export var npc_riddle: Texture2D         # 2) indovinello
@export var npc_answers: Texture2D        # 3) schermata risposte (con 1/2/3 disegnati)

@export var correct_key := 2              # 1,2,3

@export var npc_correct: Texture2D        # finale corretto
@export var npc_wrong: Texture2D          # finale sbagliato
...
func _start_dialogo():
	...
	# Sequenza: intro -> indovinello -> risposte (qui scegli 1/2/3)
	var seq: Array[VignetteLayer.Vignetta] = []
	if npc_intro:
		seq.append(VignetteLayer.Vignetta.new(npc_intro, "right"))
	if npc_riddle:
		seq.append(VignetteLayer.Vignetta.new(npc_riddle, "right"))
	if npc_answers:
		seq.append(VignetteLayer.Vignetta.new(npc_answers, "right"))

	layer.vignette = seq

	# ✅ La scelta si attiva sulla schermata "risposte" (ultima vignetta)
